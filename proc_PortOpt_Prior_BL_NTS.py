...
cons = ({'type': 'eq', 'fun': lambda x: x.sum() - 1},
        ...
        {'type': 'ineq', 'fun': lambda w: np.dot(w, muarray) - b})
res = minimize(
    lambda w:  port_CVaR_Gauss(w, muarray, cov, eta),
            x0 = eqw,
            bounds=[(0, 1) for _ in range(dim)],
            constraints = cons,
            options={"maxiter": 1000}
...
cons = ({'type': 'eq', 'fun': lambda x: x.sum() - 1},
        ...
        {'type': 'ineq', 'fun': lambda w: np.dot(w, muarray) - b})
res = minimize(
    lambda w: port_CVaR_NTS(w, strPMNTS, eta),
    x0 = eqw,
    bounds=[(0, 1) for _ in range(dim)],
    constraints = cons,
    options={"maxiter": 1000}
...
with open('data/saveddata_Portopt_Prior_BL_NTS.pickle', 'wb') as f:
    pickle.dump(data, f)